...
int init_bus_arb(bus_state*);
static int init_pr_bus(bus_state*);
static int init_d_bus(bus_state*);
...
{
	int ret;
	bus_state *bus = (bus_state*) malloc(sizeof(bus_state));
	if (bus == NULL) {
	...
	ret = pthread_mutex_init(&(bus->master_xfer_mutex), NULL);
	if (ret != 0) {
		perror("master_xfer_mutex");
		free(bus);
		return NULL;
	}

	ret = pthread_mutex_init(&(bus->slave_xfer_mutex), NULL);
	if (ret != 0) {
		perror("slave_xfer_mutex");
		free(bus);
		return NULL;
	}
...
void
release_bus_master(bus_state *bus)
{
...
void
write_data_out(bus_state *bus, uint32_t addr, uint16_t data)
{
	data_op status;
...
int
connect_device_bus(bus_state *bus)
{
	int ret;
	ret = init_pr_bus(bus);
	if (ret != 0) {
...
void
data_bus_check(bus_state *bus, data_xfer_spec *d_op)
{
	// Get slave request.
	pthread_mutex_lock(&(bus->slave_xfer_mutex));
	d_op->op = bus->slave.op;
	d_op->addr = bus->slave.addr;
	d_op->value = bus->slave.value;
	pthread_mutex_unlock(&(bus->slave_xfer_mutex));
}

void
data_bus_reply(bus_state *bus, data_xfer_spec *d_op)
{
